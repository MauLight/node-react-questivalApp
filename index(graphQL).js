...
const { PORT, MONGO_URL, _, __, ___, TEMPLATE_ID, PUBLIC_KEY, PRIVATE_KEY, SERVICE_ID, JWT_SECRET, SECRET } = require('./utils/config')
...
startStandaloneServer(server, {
  ...
  context: async ({ req }) => {
    ...
    if (auth && auth.startsWith('Bearer ')) {
      ...
      const currentUser = await User.findOne(decodedToken.email)
